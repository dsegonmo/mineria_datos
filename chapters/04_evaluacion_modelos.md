...
# y_probas: probabilidades predichas, shape (n, 2), columna 1 = prob. clase positiva
skplt.metrics.plot_cumulative_gain(y_true, y_probas)
plt.title('Curva de Ganancia Acumulada — Telco Churn')
plt.show()
```
...
plt.title('Curva de Elevación (Lift) — Telco Churn')
plt.axhline(y=1, color='gray', linestyle='--', label='Línea base (azar)')
plt.show()
```
...
# Calcular beneficio para cada umbral
...
for t in thresholds:
    ...
    beneficio = tp * (LTV - C_retencion) + fp * (-C_retencion) + fn * (-LTV)
    beneficios.append(beneficio)

# Encontrar umbral óptimo
umbral_optimo = thresholds[np.argmax(beneficios)]
beneficio_maximo = max(beneficios)

print(f"Umbral óptimo: {umbral_optimo:.2f}")
print(f"Beneficio máximo esperado: ${beneficio_maximo:,.0f}")

# Graficar
plt.figure(figsize=(10, 5))
plt.plot(thresholds, beneficios, color='steelblue', linewidth=2, label='Beneficio Esperado')
...
plt.xlabel('Umbral de Decisión ($t$)')
plt.ylabel('Beneficio Esperado ($)')
plt.title('Curva de Beneficio — Telco Churn')
plt.legend()
plt.show()
```
...
## Caso Integrador: Estrategia de Retención (Churn)

### Dataset y Contexto

Utilizaremos el **Telco Customer Churn** (IBM/Kaggle), un dataset real de una empresa de telecomunicaciones con las siguientes características:

- 7,043 clientes
- 21 variables: contrato, cargo mensual, tenure, tipo de servicio, etc.
- Variable objetivo: `Churn` (Yes/No)
- Tasa de churn: ~26.5% (moderadamente desbalanceado)

Parámetros financieros del caso:

- LTV promedio: $3,500 por cliente retenido
- Costo de campaña de retención: $150 por contacto
...
# Seleccionar features numéricas para el ejemplo base
...
### Paso 2: Accuracy Alto Pero F1 Bajo por Desbalance
...
Resultado típico con umbral estándar (0.5):

```
              precision    recall  f1-score   support

           0       0.86      0.90      0.88      1033
           1       0.65      0.55      0.60       376

    accuracy                           0.81      1409
   macro avg       0.76      0.73      0.74      1409
```

Observamos exactamente la Paradoja de la Exactitud en acción:

- Accuracy: 81% → suena bien
- Recall de clase 1 (churners): 55% → estamos perdiendo el 45% de los clientes que se van

### Paso 3: Matriz de Confusión Financiera
...
beneficio_umbral_05 = tp * (LTV - C) + fp * (-C) + fn * (-LTV)
print(f"Beneficio con umbral 0.5: ${beneficio_umbral_05:,.0f}")
```

Con umbral 0.5 típico: los FN son numerosos y cada uno cuesta $3,500.

### Paso 4: Tabla de Deciles y Lift Chart
...
resultados = pd.DataFrame({'y_real': y_test.values, 'y_prob': y_prob})
resultados = resultados.sort_values('y_prob', ascending=False).reset_index(drop=True)
resultados['decil'] = pd.qcut(resultados.index, q=10, labels=range(1, 11))

tabla_deciles = resultados.groupby('decil').agg(
    clientes=('y_real', 'count'),
    churners=('y_real', 'sum')
).reset_index()

tasa_base = y_test.mean()
tabla_deciles['pct_churners'] = tabla_deciles['churners'] / tabla_deciles['clientes']
tabla_deciles['lift'] = tabla_deciles['pct_churners'] / tasa_base
print(tabla_deciles)
```

### Paso 5: Curva de Beneficio y Umbral Óptimo
...
for t in thresholds:
    ...
    b = tp_t * (LTV - C) + fp_t * (-C) + fn_t * (-LTV)
    beneficios.append(b)

idx_optimo = np.argmax(beneficios)
umbral_optimo = thresholds[idx_optimo]
beneficio_optimo = beneficios[idx_optimo]

print(f"Umbral óptimo: {umbral_optimo:.2f}")
print(f"Beneficio con umbral óptimo: ${beneficio_optimo:,.0f}")
print(f"Beneficio con umbral 0.5: ${beneficio_umbral_05:,.0f}")
print(f"Diferencia: ${beneficio_optimo - beneficio_umbral_05:,.0f}")
```

### Paso 6: Comparación Final

| Configuración              | Umbral | Accuracy | Recall | Beneficio Esperado  |
|----------------------------|--------|----------|--------|---------------------|
| Umbral estándar             | 0.50   | 81%      | 55%    | $XXX,000            |
| **Umbral óptimo financiero**| **~0.18** | **73%** | **85%** | **$XXX,000 (+30%)**  |

El umbral óptimo tiene **menor exactitud** pero capta el 85% de los churners. Con un LTV de $3,500, cada churner adicional detectado vale 23 veces el costo de un FP ($3,500 / $150 ≈ 23x). La decisión financieramente correcta es clara: bajar el umbral aunque "duela" en la métrica de Accuracy.
